...
| [ ] | `OrderPolicyPageLoaderTest.php` | 22 | Policy-report inputs, wiring, configuration и error states | Discount Abuse, Same IP, Tag Policy, Duplicate Shipping Addresses и Note Flags paths са покрити; останалите policy reports чакат method-level сверка |
...
| [ ] | `UserActionLogTest.php` | 3 | Action history, pruning и legacy import → DB audit log |
