...
/**
 * Itemized quantity report for refunded line items in a date range.
 */
final class ReturnedItemsReport
{
    ...
    public static function saveCsv(array $totals, string $startDate, string $endDate, string $dir = ''): string
    {
        ...
        $path = "{$dir}/returned_items_{$startDate}_to_{$endDate}.csv";
        $tmp  = tempnam($dir, 'returned_items.csv.tmp.');
        if ($tmp === false) {
    ...
    public static function emailHtml(array $totals, string $startDate, string $endDate): string
    {
        ...
        return '<!DOCTYPE html><html><head><meta charset="utf-8"></head>'
            ...
            . '<h2 style="margin-bottom:4px">Returned Items Report</h2>'
            . '<p style="margin:0 0 16px;color:#555">' . self::h($startDate) . ' &rarr; ' . self::h($endDate) . '</p>'
